<div class="border-bottom bg-white d-flex align-items-center justify-content-between sticky-top app-subpage-header px-3 py-2">
    <h6 class="fw-bold m-0 text-dark" style="font-size: 14px; letter-spacing: -0.3px;">Pesanan Saya</h6>
    <span id="order-count-badge" class="badge text-white px-2.5 py-1 rounded-pill shadow-2xs" style="background: linear-gradient(135deg, #EE2737, #C61524); font-size: 9.5px;">
    ...
    <?php else: ?>
        <div class="d-flex flex-column gap-3" id="orders-list-wrapper">
            <?php foreach ($orders as $order): ?>
                ...
                $isUnpaid = (in_array($order['payment_method'], ['doku', 'online']) && $order['payment_status'] !== 'paid' && !$isCanceled);
...
(function() {
    function getStatusMeta(status, paymentMethod, paymentStatus) {
        ...
        const isUnpaid = (['doku', 'online'].includes(paymentMethod) && paymentStatus !== 'paid' && !isCanceled);
