<div class="border-bottom bg-white d-flex align-items-center gap-2.5 sticky-top shadow-xs px-3.5 py-3">
    ...
    <h6 class="fw-bold m-0 text-dark" style="font-size: 14px;">Konfirmasi & Pembayaran</h6>
</div>

<form id="checkoutForm" onsubmit="handlePlaceOrder(event)" class="px-3.5 pt-4 pb-5" style="min-height: 85vh;">
    <!-- Map Location Picker Card -->
    <div class="p-3.5 bg-white border shadow-xs mb-4" style="border-radius: 16px;">
        <div class="d-flex align-items-center justify-content-between mb-2.5">
            <h6 class="fw-bold m-0 text-dark" style="font-size: 13px;"><i class="bi bi-geo-alt-fill text-danger me-1"></i> Titik Lokasi Antar</h6>
            <button type="button" onclick="getCurrentLocation()" class="btn btn-sm rounded-pill py-1.5 px-3 fw-bold text-white shadow-xs" style="background:#EE2737; font-size: 10.5px;">
                <i class="bi bi-crosshair me-0.5"></i> GPS Saya
            </button>
        </div>
        <div class="text-muted mb-2.5" style="font-size: 10.5px; line-height: 1.35;">
            Geser pin merah atau ketuk peta untuk menyesuaikan lokasi tepat rumah Anda.
        </div>
        <div id="checkout-map" style="width: 100%; height: 180px; border-radius: 12px;" class="border shadow-xs mb-2.5"></div>
        <div class="d-flex align-items-center justify-content-between">
            <span class="badge bg-light text-dark border py-1.5 px-2.5" id="distance-badge" style="font-size: 9.5px;"><i class="bi bi-signpost-2 me-1"></i> Est. Jarak: 1.5 Km</span>
            <span class="badge bg-danger-subtle text-danger py-1.5 px-2.5" id="zone-badge" style="font-size: 9.5px;"><i class="bi bi-shield-check me-1"></i> Tercover Zona Cicalengka</span>
        </div>
    ...
    <div class="p-3.5 bg-white border shadow-xs mb-4" style="border-radius: 16px;">
        <div class="mb-2.5">
            <label class="form-label text-muted mb-1.5" style="font-size: 11px; font-weight: 600;">Alamat Lengkap / Patokan Rumah</label>
            <textarea name="address" id="input-address" class="form-control form-control-sm bg-light" rows="2" style="font-size: 11.5px; border-radius: 10px; padding: 8px 12px;" required placeholder="Jl. Raya Cicalengka No. 45 (Dekat Stasiun / Rumah Cat Hijau)">Jl. Cicalengka Raya No. 45, RT 02/03</textarea>
        </div>
        <div class="row g-2.5">
            <div class="col-6">
                <label class="form-label text-muted mb-1.5" for="contact_name" style="font-size: 11px; font-weight: 600;">Nama Penerima</label>
                <input type="text" name="contact_name" id="contact_name" class="form-control form-control-sm bg-light" style="font-size: 11.5px; border-radius: 10px; padding: 8px 12px;" value="<?= htmlspecialchars($_SESSION['user']['name'] ?? '') ?>" required>
            </div>
            <div class="col-6">
                <label class="form-label text-muted mb-1.5" for="contact_phone" style="font-size: 11px; font-weight: 600;">No. WhatsApp</label>
                <input type="text" name="contact_phone" id="contact_phone" class="form-control form-control-sm bg-light" style="font-size: 11.5px; border-radius: 10px; padding: 8px 12px;" value="<?= htmlspecialchars($_SESSION['user']['phone'] ?? '') ?>" required>
            </div>
        </div>
    </div>
    ...
    <div class="p-3.5 bg-white border shadow-xs mb-4" style="border-radius: 16px;">
        <h6 class="fw-bold mb-3 text-dark" style="font-size: 13px;"><i class="bi bi-wallet2 text-danger me-1"></i> Metode Pembayaran</h6>

        <div class="d-flex flex-column gap-2.5">
            <!-- CicalengkaPay Digital Wallet -->
            <label class="p-3 border rounded-3 d-flex align-items-center justify-content-between cursor-pointer payment-option <?= ((float)$wallet['balance'] >= (float)$cart_data['subtotal']) ? 'border-danger bg-danger-subtle' : 'opacity-75' ?>" style="cursor: pointer; border-radius: 12px !important;">
                <div class="d-flex align-items-center gap-2.5">
                    ...
                    <div>
                        <div class="fw-bold d-flex align-items-center gap-1.5" style="font-size: 12px;">
                            <span style="color:#EE2737;font-weight:800;">CicalengkaPay</span>
                            <span class="text-muted" style="font-size: 10px;">(Saldo Digital)</span>
                        </div>
                        <div class="text-muted" style="font-size: 10px;">Saldo: <?= format_rupiah($wallet['balance'] ?? 0) ?></div>
                    </div>
                </div>
                <?php if ((float)$wallet['balance'] < (float)$cart_data['subtotal']): ?>
                    <span class="badge bg-warning text-dark py-1 px-2.5 rounded-pill" style="font-size: 9px;">Kurang</span>
                <?php else: ?>
                    <span class="badge bg-danger py-1 px-2.5 rounded-pill text-white" style="background:#EE2737 !important; font-size: 9px;">Tersedia</span>
                <?php endif; ?>
            </label>

            <!-- Midtrans Online Payment (QRIS / VA / E-Wallet) -->
            <label class="p-3 border rounded-3 d-flex align-items-center justify-content-between cursor-pointer payment-option" style="cursor: pointer; border-radius: 12px !important;">
                <div class="d-flex align-items-center gap-2.5">
                    <input type="radio" name="payment_method" id="pay_midtrans" value="midtrans">
                    <div>
                        <div class="fw-bold d-flex align-items-center gap-1.5 text-dark" style="font-size: 12px;">
                            <span>Bayar Online (Midtrans)</span>
                            <span class="badge bg-danger-subtle text-danger py-0.5 px-2" style="font-size: 9px; font-weight: 700;">Otomatis</span>
                        </div>
                        <div class="text-muted" style="font-size: 10px;">QRIS, GoPay, ShopeePay, VA Bank</div>
                    </div>
                </div>
            ...
            <label class="p-3 border rounded-3 d-flex align-items-center justify-content-between cursor-pointer payment-option" style="cursor: pointer; border-radius: 12px !important;">
                <div class="d-flex align-items-center gap-2.5">
                    ...
                    <div>
                        <div class="fw-bold text-dark" style="font-size: 12px;">Tunai saat Tiba (COD)</div>
                        <div class="text-muted" style="font-size: 10px;">Bayar langsung ke kurir motor</div>
                    </div>
                </div>
    ...
    <!-- Voucher / Coupon Code -->
    <div class="p-3.5 bg-white border shadow-xs mb-4" style="border-radius: 16px;">
        <h6 class="fw-bold mb-2.5 text-dark" style="font-size: 12.5px;"><i class="bi bi-percent text-warning me-1"></i> Promo & Kupon</h6>
        <div class="input-group input-group-sm">
            <input type="text" name="coupon_code" id="coupon_code" class="form-control bg-light" style="font-size: 11.5px; border-radius: 10px 0 0 10px; padding: 8px 12px;" placeholder="Kode promo (Contoh: CCGHEMAT)">
            <button type="button" class="btn text-white fw-bold px-3.5" style="background:#EE2737; font-size: 11.5px; border-radius: 0 10px 10px 0;" onclick="applyCouponPreview()">Pakai</button>
        </div>
    </div>

    <!-- Order Notes -->
    <div class="p-3.5 bg-white border shadow-xs mb-4" style="border-radius: 16px;">
        <label class="form-label fw-bold mb-1.5 d-block text-dark" for="order_notes" style="font-size: 12.5px;"><i class="bi bi-chat-left-text me-1 text-muted"></i> Catatan Pesanan</label>
        <input type="text" name="order_notes" id="order_notes" class="form-control form-control-sm bg-light" style="font-size: 11.5px; border-radius: 10px; padding: 8px 12px;" placeholder="Contoh: Sambal dipisah, jangan pakai bawang goreng">
    </div>

    <!-- Order Breakdown Card -->
    <div class="p-3.5 bg-white border shadow-xs mb-4" style="border-radius: 16px;">
        <h6 class="fw-bold mb-3 text-dark" style="font-size: 13px;">Rincian Tagihan</h6>
