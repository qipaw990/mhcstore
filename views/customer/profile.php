...
<div class="p-2" style="padding: 8px 10px !important;">
    <?php if (!empty($_SESSION['error'])): ?>
        <div class="alert alert-danger rounded-3 border-0 p-2 mb-2" style="font-size: 10px;">
    ...
    <?php if (!empty($_SESSION['success'])): ?>
        <div class="alert alert-success rounded-3 border-0 p-2 mb-2" style="font-size: 10px;">
    ...
    <?php if ($user): ?>
        ...
        <div class="p-2.5 bg-white rounded-3 border shadow-xs mb-2 d-flex align-items-center gap-2.5">
            <div class="position-relative">
                <img src="<?= $baseUrl ?>/<?= htmlspecialchars($user['avatar'] ?? 'assets/images/users/default.png') ?>" alt="User" class="rounded-circle border border-2" style="width: 42px; height: 42px; object-fit: cover; border-color: #EE2737 !important;">
            </div>
            <div class="flex-grow-1 min-w-0">
                <div class="fw-bold text-truncate" style="color: var(--gojek-charcoal); font-size: 12px;"><?= htmlspecialchars($user['name'] ?? 'Pengguna') ?></div>
                <div class="text-muted" style="font-size: 9.5px;"><?= htmlspecialchars($user['phone'] ?? '-') ?></div>
                <?php if (!empty($user['email'])): ?>
                    <div class="text-muted text-truncate" style="font-size: 9px;"><?= htmlspecialchars($user['email']) ?></div>
                <?php endif; ?>
            </div>
            <button type="button" class="btn btn-light btn-sm rounded-circle border shadow-xs d-flex align-items-center justify-content-center" style="width: 28px; height: 28px;" data-bs-toggle="modal" data-bs-target="#editProfileModal" title="Edit Profil">
                <i class="bi bi-pencil-fill text-danger" style="font-size: 11px;"></i>
            </button>
        </div>

        <!-- CicalengkaPay Quick Card -->
        <div class="p-2.5 bg-white rounded-3 border shadow-xs mb-2 d-flex align-items-center justify-content-between">
            <div class="d-flex align-items-center gap-2">
                <div class="rounded-circle d-flex align-items-center justify-content-center text-white" style="width: 32px; height: 32px; font-size: 14px; background: linear-gradient(135deg, #EE2737, #C61524); box-shadow: 0 2px 6px rgba(238,39,55,0.25);">
                    <i class="bi bi-wallet2"></i>
                </div>
                <div>
                    <div class="fw-bold" style="color: #EE2737; font-size: 9.5px;">Cicalengka<span style="color:#C61524;">Pay</span></div>
                    <div class="fw-extrabold text-dark" style="font-size: 12px;"><?= format_rupiah($wallet['balance'] ?? 0) ?></div>
                </div>
            </div>
            <a href="<?= $baseUrl ?>/wallet" class="btn btn-sm rounded-pill fw-bold px-2.5 py-1 text-white shadow-xs" style="background:#EE2737; font-size: 9.5px;">
                Isi Saldo
            </a>
        </div>

        <!-- Menu Navigation List -->
        <div class="bg-white rounded-3 border shadow-xs overflow-hidden mb-2">
            <a href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#editProfileModal" class="p-2.5 d-flex align-items-center justify-content-between text-decoration-none text-dark border-bottom">
                <div class="d-flex align-items-center gap-2">
                    <i class="bi bi-person-gear" style="color: #EE2737 !important; font-size: 14px;"></i>
                    <div>
                        <div class="fw-bold" style="font-size: 10.5px;">Edit Profil & Kata Sandi</div>
                        <div class="text-muted" style="font-size: 9px;">Ubah foto, profil & password</div>
                    </div>
                </div>
                <i class="bi bi-chevron-right text-muted" style="font-size: 10px;"></i>
            </a>

            <a href="<?= $baseUrl ?>/orders" class="p-2.5 d-flex align-items-center justify-content-between text-decoration-none text-dark border-bottom">
                <div class="d-flex align-items-center gap-2">
                    <i class="bi bi-receipt" style="color: #EE2737 !important; font-size: 14px;"></i>
                    <span class="fw-bold" style="font-size: 10.5px;">Riwayat Pesanan</span>
                </div>
                <i class="bi bi-chevron-right text-muted" style="font-size: 10px;"></i>
            </a>

            <a href="<?= $baseUrl ?>/search" class="p-2.5 d-flex align-items-center justify-content-between text-decoration-none text-dark border-bottom">
                <div class="d-flex align-items-center gap-2">
                    <i class="bi bi-percent text-warning" style="font-size: 14px;"></i>
                    <span class="fw-bold" style="font-size: 10.5px;">Voucher & Promo Saya</span>
                </div>
                <i class="bi bi-chevron-right text-muted" style="font-size: 10px;"></i>
            </a>

            <a href="<?= $baseUrl ?>/notifications" class="p-2.5 d-flex align-items-center justify-content-between text-decoration-none text-dark border-bottom">
                <div class="d-flex align-items-center gap-2">
                    <i class="bi bi-bell text-primary" style="font-size: 14px;"></i>
                    <span class="fw-bold" style="font-size: 10.5px;">Pusat Notifikasi</span>
                </div>
                <i class="bi bi-chevron-right text-muted" style="font-size: 10px;"></i>
            </a>

            <a href="javascript:void(0)" onclick="Swal.fire('Pusat Bantuan', 'Hubungi layanan pelanggan CicalengkaGO via WhatsApp di 0812-3456-7890', 'info')" class="p-2.5 d-flex align-items-center justify-content-between text-decoration-none text-dark border-bottom">
                <div class="d-flex align-items-center gap-2">
                    <i class="bi bi-question-circle-fill text-info" style="font-size: 14px;"></i>
                    <span class="fw-bold" style="font-size: 10.5px;">Bantuan & CS 24 Jam</span>
                </div>
                <i class="bi bi-chevron-right text-muted" style="font-size: 10px;"></i>
            </a>

            <a href="<?= $baseUrl ?>/logout" class="p-2.5 d-flex align-items-center justify-content-between text-decoration-none text-danger">
                <div class="d-flex align-items-center gap-2">
                    <i class="bi bi-box-arrow-right" style="font-size: 14px;"></i>
                    <span class="fw-bold" style="font-size: 10.5px;">Keluar Akun</span>
                </div>
    ...
    <?php else: ?>
        <div class="text-center py-4">
            ...
            <h6 class="fw-bold mb-1" style="color: var(--gojek-charcoal); font-size: 12.5px;">Masuk ke Akun CicalengkaGO</h6>
            <p class="text-muted" style="font-size: 10px;">Masuk untuk melihat pesanan, saldo CicalengkaPay, dan promo khusus Anda.</p>
            <div class="d-grid gap-1.5 mt-2.5">
                <a href="<?= $baseUrl ?>/login" class="btn btn-gojek-green py-1.5" style="background:#EE2737 !important; color:#FFFFFF !important; border-radius:9999px; font-weight:700; font-size:11.5px; text-decoration:none;">Masuk Sekarang</a>
                <a href="<?= $baseUrl ?>/register" class="btn btn-outline-danger rounded-pill fw-bold py-1.5" style="border-color:#EE2737; color:#EE2737; font-size:11.5px;">Daftar Akun Baru</a>
            </div>
        </div>
    <?php endif; ?>
</div>
