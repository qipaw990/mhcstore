...
<div class="gojek-services-section">
    <div class="d-flex align-items-center justify-content-between mb-3 px-1">
        <h6 class="fw-bold m-0" style="font-size: 15px; color: var(--gojek-charcoal); letter-spacing: -0.3px;">
            <i class="bi bi-grid-1x2-fill me-1" style="color: var(--gojek-green);"></i> Kategori Pilihan
        </h6>
        <a href="<?= $baseUrl ?>/search" class="text-decoration-none fw-bold" style="font-size: 12px; color: var(--gojek-green);">
...
<div class="gojek-promo-section">
    <div id="gojekBannerCarousel" class="carousel slide gojek-carousel-container" data-bs-ride="carousel">
        <div class="carousel-inner">
            <?php foreach ($banners as $index => $banner): ?>
                <div class="carousel-item <?= $index === 0 ? 'active' : '' ?>">
                    <img src="<?= $baseUrl ?>/<?= htmlspecialchars($banner['image']) ?>" alt="<?= htmlspecialchars($banner['title']) ?>" class="d-block w-100">
                </div>
            <?php endforeach; ?>
        </div>
...
<div class="gojek-section-header">
    <div>
        <h2 class="gojek-section-title">Paling Laris di Cicalengka</h2>
        <div class="gojek-section-subtitle">Diskon & resto favorit pilihan warga Cicalengka</div>
    </div>
    <a href="<?= $baseUrl ?>/search?module_id=<?= $selected_module_id ?>" class="gojek-section-link">Lihat Semua</a>
</div>

<div class="gofood-stores-scroll">
    ...
    <?php else: ?>
        <?php foreach ($popular_stores as $store): ?>
            <a href="<?= $baseUrl ?>/stores/<?= $store['id'] ?>" class="gofood-store-card">
                <div class="gofood-store-img-box">
                    ...
                    <?php else: ?>
                        <span class="gofood-closed-tag"><i class="bi bi-door-closed-fill me-1"></i> Tutup</span>
                    <?php endif; ?>
                </div>
                <div class="gofood-store-body">
                    <div class="gofood-store-name"><?= htmlspecialchars($store['name']) ?></div>
...
<div class="gojek-section-header">
    <div>
        <h2 class="gojek-section-title">Rekomendasi Menu Favorit</h2>
        <div class="gojek-section-subtitle">Paling sering dipesan dengan pengantaran cepat</div>
    </div>
</div>

<div class="gofood-products-grid">
