...
<div class="px-3 py-2.5">
    <?php if (empty($cart_summary['items'])): ?>
        <div class="text-center py-5">
            ...
            <h6 class="fw-bold mb-1.5" style="color: var(--gojek-charcoal); font-size: 13px;">Keranjang Anda Masih Kosong</h6>
            <p class="text-muted mb-3" style="font-size: 11px;">Yuk cari kuliner lezat atau belanja kebutuhan Anda sekarang.</p>
            <a href="<?= $baseUrl ?>" class="btn btn-gojek-green px-4 py-2" style="background:#EE2737 !important; color:#FFFFFF !important; border-radius:9999px; width: auto; display: inline-flex; text-decoration:none; font-size: 11.5px; font-weight: 700;">Mulai Belanja</a>
        </div>
    <?php else: ?>
        <!-- Store Name Header -->
        <div class="p-3 bg-white border shadow-xs mb-2.5 d-flex align-items-center gap-2.5" style="border-radius: 14px;">
            ...
            <div class="min-w-0">
                <div class="fw-bold text-truncate text-dark" style="font-size: 12px;"><?= htmlspecialchars($cart_summary['store']['name']) ?></div>
                <div class="text-muted" style="font-size: 10px;">Estimasi pengantaran 20-30 mnt</div>
            </div>
        </div>
            ...
            <?php foreach ($cart_summary['items'] as $item): ?>
                <div class="p-3 bg-white border shadow-xs d-flex align-items-center justify-content-between gap-3" style="border-radius: 14px;">
                    ...
                    <div class="flex-grow-1 min-w-0">
                        <div class="fw-bold text-dark text-truncate" style="font-size: 11.5px;"><?= htmlspecialchars($item['product_name']) ?></div>
                        <div class="fw-bold text-danger mt-0.5" style="font-size: 11.5px;"><?= format_rupiah($item['price']) ?></div>
                    </div>

                    <!-- Quantity Modifiers -->
                    <div class="d-flex align-items-center gap-1.5 bg-light p-1 rounded-pill border">
                        <button onclick="updateCartQty(<?= $item['id'] ?>, -1)" class="btn btn-sm btn-white p-0 d-flex align-items-center justify-content-center text-dark" style="width: 24px; height: 24px; border-radius: 50%; font-size: 11px;">
                            <i class="bi bi-dash"></i>
                        </button>
                        <span class="fw-bold px-1.5" style="font-size: 11px;"><?= $item['quantity'] ?></span>
                        <button onclick="updateCartQty(<?= $item['id'] ?>, 1)" class="btn btn-sm btn-white p-0 d-flex align-items-center justify-content-center text-white" style="width: 24px; height: 24px; background: #EE2737; border-radius: 50%; font-size: 11px;">
        ...
        <div class="p-3 bg-white border shadow-xs mb-3" style="border-radius: 14px;">
            <h6 class="fw-bold mb-2.5 text-dark" style="font-size: 12px;">Ringkasan Pembayaran</h6>
        ...
        <a href="<?= $baseUrl ?>/checkout" class="btn btn-gojek-green w-100" style="background:#EE2737 !important; color:#FFFFFF !important; border-radius:9999px; font-weight:700; padding:10px 16px; font-size:12px; box-shadow:0 3px 10px rgba(238,39,55,0.3); text-decoration:none; display:flex; align-items:center; justify-content:center; gap:8px;">
