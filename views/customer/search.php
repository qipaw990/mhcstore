...
<div class="bg-white border-bottom sticky-top shadow-2xs px-3 pt-3 pb-2.5" style="z-index: 1020;">
    <div class="d-flex align-items-center gap-2.5 w-100">
        ...
        <form action="<?= $baseUrl ?>/search" method="GET" class="flex-grow-1 m-0 position-relative min-w-0">
            <div class="m-0 d-flex align-items-center gap-2 px-3 py-1.5 rounded-pill overflow-hidden" style="background: #F1F5F9; border: 1px solid #E2E8F0; height: 38px;">
                <i class="bi bi-search text-danger flex-shrink-0" style="font-size: 14px;"></i>
                <input type="text" name="q" id="search-input" value="<?= htmlspecialchars($query ?? '') ?>" placeholder="Cari sate, geprek, martabak, sembako..." autocomplete="off" style="border: none; background: transparent; outline: none; font-size: 12px; width: 100%; font-weight: 500; color: #1E293B;">
                <?php if (!empty($query)): ?>
...
<div class="px-3 py-3.5 d-flex flex-column gap-3.5">
    <?php if (empty($query)): ?>
        <!-- Trending & Popular Searches -->
        <div>
            <h6 class="fw-bold mb-2.5 text-dark d-flex align-items-center gap-1.5" style="font-size: 12.5px;">
                <i class="bi bi-fire text-danger" style="font-size: 14px;"></i> Pencarian Populer
            </h6>
            <div class="d-flex flex-wrap gap-2">
                <button type="button" onclick="quickSearch('Ayam Geprek')" class="btn btn-sm bg-white rounded-pill px-3 py-1.5 border text-dark fw-semibold shadow-2xs" style="font-size: 11px; border-color: #E2E8F0 !important;">🍗 Ayam Geprek</button>
                <button type="button" onclick="quickSearch('Sate Maranggi')" class="btn btn-sm bg-white rounded-pill px-3 py-1.5 border text-dark fw-semibold shadow-2xs" style="font-size: 11px; border-color: #E2E8F0 !important;">🍢 Sate Maranggi</button>
                <button type="button" onclick="quickSearch('Seblak Pedas')" class="btn btn-sm bg-white rounded-pill px-3 py-1.5 border text-dark fw-semibold shadow-2xs" style="font-size: 11px; border-color: #E2E8F0 !important;">🌶️ Seblak</button>
                <button type="button" onclick="quickSearch('Martabak')" class="btn btn-sm bg-white rounded-pill px-3 py-1.5 border text-dark fw-semibold shadow-2xs" style="font-size: 11px; border-color: #E2E8F0 !important;">🥞 Martabak</button>
                <button type="button" onclick="quickSearch('Beras')" class="btn btn-sm bg-white rounded-pill px-3 py-1.5 border text-dark fw-semibold shadow-2xs" style="font-size: 11px; border-color: #E2E8F0 !important;">🍚 Sembako</button>
                <button type="button" onclick="quickSearch('Kopi')" class="btn btn-sm bg-white rounded-pill px-3 py-1.5 border text-dark fw-semibold shadow-2xs" style="font-size: 11px; border-color: #E2E8F0 !important;">☕ Kopi Susu</button>
            </div>
        </div>

        <!-- Quick Promo Banner -->
        <div class="shadow-2xs text-white p-3.5 position-relative overflow-hidden" style="background: linear-gradient(135deg, #EE2737 0%, #B91C1C 100%); border-radius: 16px;">
        ...
        <?php if (!empty($popular_stores)): ?>
            <div>
                <div class="d-flex align-items-center justify-content-between mb-2.5">
                    <h6 class="fw-bold m-0 text-dark d-flex align-items-center gap-1.5" style="font-size: 12.5px;">
                        <i class="bi bi-shop text-danger" style="font-size: 14px;"></i> Resto Populer di Cicalengka
                    </h6>
        ...
        <?php if (!empty($recommend_products)): ?>
            <div>
                <div class="d-flex align-items-center justify-content-between mb-2.5">
                    <h6 class="fw-bold m-0 text-dark d-flex align-items-center gap-1.5" style="font-size: 12.5px;">
                        <i class="bi bi-stars text-warning" style="font-size: 14px;"></i> Rekomendasi Menu Favorit
                    </h6>
                </div>
    ...
    <?php else: ?>
        ...
        <?php if (!empty($stores)): ?>
            <div>
                <div class="d-flex align-items-center justify-content-between mb-2.5">
                    <h6 class="fw-bold m-0 text-dark" style="font-size: 12.5px;"><i class="bi bi-shop text-danger me-1"></i> Resto & Toko (<?= count($stores) ?>)</h6>
                </div>
                <div class="gofood-stores-scroll mb-0 p-0">
        ...
        <div>
            <div class="d-flex align-items-center justify-content-between mb-2.5">
                <h6 class="fw-bold m-0 text-dark" style="font-size: 12.5px;"><i class="bi bi-egg-fried text-primary me-1"></i> Menu & Produk (<?= count($products) ?>)</h6>
            </div>
