...
<div class="border-bottom bg-white d-flex align-items-center justify-content-between sticky-top shadow-xs px-3.5 py-3">
    <div class="d-flex align-items-center gap-2.5">
        <a href="<?= $baseUrl ?>/orders" class="btn btn-light btn-sm rounded-circle d-flex align-items-center justify-content-center" style="width: 36px; height: 36px; font-size: 15px; border: 1px solid #E2E8F0; background: #F8FAFC;"><i class="bi bi-arrow-left"></i></a>
        <div>
            <h6 class="fw-bold m-0" style="color: var(--gojek-charcoal); font-size: 14px;">
    ...
    <?php if ($isCanceled): ?>
        <span id="order-status-badge" class="badge px-3 py-1.5 text-uppercase text-white fw-bold bg-danger" style="font-size: 10px; border-radius: 6px;">
            DIBATALKAN
        </span>
    <?php elseif ($isUnpaidOnline): ?>
        <span id="order-status-badge" class="badge px-3 py-1.5 text-uppercase bg-warning text-dark fw-bold border border-warning d-flex align-items-center gap-1" style="font-size: 10px; border-radius: 6px;">
            <i class="bi bi-clock-history"></i> MENUNGGU BAYAR
        </span>
    <?php else: ?>
        <span id="order-status-badge" class="badge px-3 py-1.5 text-uppercase fw-bold <?= $currentBadge['class'] ?>" style="font-size: 10px; border-radius: 6px;">
            <?= $currentBadge['label'] ?>
        </span>
    <?php endif; ?>
</div>
...
<div class="px-3.5 pt-4 pb-5" style="min-height: 85vh;">
    <?php if ($isCanceled): ?>
        <!-- Canceled Card -->
        <div class="p-4 bg-white border shadow-xs text-center mb-4" style="border-radius: 16px;">
            <div class="rounded-circle bg-danger-subtle text-danger d-flex align-items-center justify-content-center mx-auto mb-3" style="width: 52px; height: 52px; font-size: 24px;">
                <i class="bi bi-x-circle-fill"></i>
            </div>
            <h6 class="fw-bold text-dark mb-1" style="font-size: 14px;">Pesanan Telah Dibatalkan</h6>
            <p class="text-muted mb-3.5" style="font-size: 11.5px;">Pesanan ini tidak lagi diproses.</p>
            <a href="<?= $baseUrl ?>" class="btn btn-sm rounded-pill px-4 py-2 fw-bold text-white shadow-xs" style="background:#EE2737; font-size: 12px;">Pesan Kembali</a>
        </div>

    <?php elseif ($isUnpaidOnline): ?>
        <!-- Unpaid Online Order Screen -->
        <div class="p-4 bg-white border shadow-xs mb-4 text-center" style="border-radius: 16px;">
            <div class="rounded-circle bg-danger-subtle text-danger d-flex align-items-center justify-content-center mx-auto mb-3" style="width: 58px; height: 58px; font-size: 26px;">
                <i class="bi bi-credit-card-2-front-fill text-danger"></i>
            </div>
            <div class="badge bg-warning-subtle text-warning-emphasis border border-warning px-3 py-1.5 rounded-pill fw-bold mb-2.5" style="font-size: 10.5px;">
                <i class="bi bi-hourglass-split me-0.5"></i> Menunggu Pembayaran
            </div>
            <h6 class="fw-bold text-dark mb-1" style="font-size: 14px;">Selesaikan Pembayaran</h6>
            <div class="text-muted mb-3" style="font-size: 11.5px;">Total tagihan yang harus dibayar:</div>
            
            <div class="p-3 bg-light rounded-3 border mb-3">
                <div class="text-muted" style="font-size: 10.5px;">TOTAL NOMINAL</div>
                <div class="fw-bold text-danger my-1.5" style="font-size: 22px;"><?= format_rupiah($order['total_amount']) ?></div>
                <div class="badge bg-white text-dark border px-2.5 py-1" style="font-size: 10.5px;">
                    <i class="bi bi-shield-check text-success me-0.5"></i> Midtrans QRIS / VA / E-Wallet
                </div>
            </div>

            <div class="alert alert-warning border-0 rounded-3 text-start p-3 mb-3 d-flex align-items-start gap-2.5" style="font-size: 11px;">
                <i class="bi bi-exclamation-triangle-fill text-warning flex-shrink-0 mt-0.5" style="font-size: 14px;"></i>
                <div>
            ...
            <!-- Action Buttons -->
            <button type="button" id="btnPayNow" onclick="payNow()" class="btn w-100 py-3 rounded-pill fw-bold text-white shadow-xs mb-2.5 d-flex align-items-center justify-content-center gap-2" style="background:#EE2737; font-size: 13px;">
                <i class="bi bi-wallet2"></i>
                <span>Bayar Sekarang (Midtrans)</span>
            </button>

            <div class="d-flex gap-2">
                <button type="button" id="btnCheckStatus" onclick="checkPaymentStatus()" class="btn btn-light border rounded-pill flex-grow-1 py-2 fw-semibold text-dark" style="font-size: 11.5px;">
                    <i class="bi bi-arrow-repeat me-0.5"></i> Cek Status Bayar
                </button>
                <button type="button" onclick="cancelUnpaidOrder()" class="btn btn-outline-danger rounded-pill px-3.5 py-2 fw-semibold" style="font-size: 11.5px;">
    ...
    <?php else: ?>
        ...
        <div class="position-relative mb-4 overflow-hidden shadow-xs border" style="border-radius: 16px;">
            <!-- Live Floating HUD (Top) -->
            <div class="position-absolute top-0 start-0 end-0 p-2.5 z-3 d-flex align-items-center justify-content-between pointer-events-none">
                <div class="badge bg-dark bg-opacity-75 backdrop-blur text-white px-3 py-1.5 fw-semibold rounded-pill d-flex align-items-center gap-1.5 shadow-xs" id="live-radar-status" style="font-size: 10px;">
                    <span class="live-dot me-0.5"></span> Live GPS Radar
                </div>
                <button onclick="toggleDriverSimulation('<?= $order['order_code'] ?>')" class="btn btn-warning btn-sm fw-bold shadow-xs rounded-pill py-1 px-3 pointer-events-auto text-dark" style="font-size: 10px;">
                    <i class="bi bi-play-circle-fill me-0.5"></i> Demo Kurir
                </button>
            </div>

            <div id="tracking-map" style="width: 100%; height: 230px;"></div>

            <!-- Floating Telemetry HUD (Bottom) -->
            <div class="position-absolute bottom-0 start-0 end-0 p-2.5 z-3 d-flex flex-column gap-2 pointer-events-none">
                <div class="d-flex align-items-center justify-content-between gap-2 pointer-events-auto">
                    <div class="bg-white bg-opacity-90 backdrop-blur border rounded-pill px-3 py-1.5 shadow-xs fw-bold text-dark d-flex align-items-center" id="live-distance-text" style="font-size: 10.5px;">
                        <i class="bi bi-pin-map-fill text-danger me-1"></i> Menghitung jarak...
                    </div>
                    <div class="text-white rounded-pill px-3 py-1.5 shadow-xs fw-bold d-flex align-items-center" style="background:#EE2737; font-size: 10.5px;" id="live-eta-text">
                        <i class="bi bi-stopwatch-fill me-1"></i> Est. Tiba
                    </div>
                </div>

                <!-- Quick Action Floating Buttons -->
                <div class="d-flex justify-content-end gap-2 mt-0.5 pointer-events-auto">
                    <button onclick="centerOnDriver()" class="btn btn-white btn-sm bg-white shadow-xs rounded-pill px-3 py-1.5 fw-semibold" style="font-size: 10.5px;">
                        <i class="bi bi-crosshair me-0.5" style="color: #EE2737;"></i> Fokus Kurir
                    </button>
                    <button onclick="openGoogleMapsNav()" class="btn btn-sm shadow-xs rounded-pill px-3 py-1.5 fw-semibold text-white" style="background: #EE2737; font-size: 10.5px;">
        ...
        <div id="order-completed-card" class="p-3.5 mb-4 text-white shadow-xs <?= $order['order_status'] === 'delivered' ? '' : 'd-none' ?>" style="background: linear-gradient(135deg, #10B981 0%, #047857 100%); border-left: 4px solid #059669; border-radius: 16px;">
            <div class="d-flex align-items-center gap-3">
                <div class="rounded-circle bg-white text-success d-flex align-items-center justify-content-center flex-shrink-0 shadow-xs" style="width: 40px; height: 40px; font-size: 20px;">
                    <i class="bi bi-check-circle-fill"></i>
                </div>
                <div>
                    <h6 class="fw-bold m-0 text-white" style="font-size: 13px;">Pesanan Selesai Diantar! 🎉</h6>
                    <div class="text-white-50" style="font-size: 10.5px; line-height: 1.3;">Terima kasih telah memesan melalui CicalengkaGO.</div>
                </div>
            </div>
        </div>

        <!-- OTP Code Banner Card (Auto-Synced) -->
        <div id="otp-banner-card" class="p-3.5 mb-4 text-white shadow-xs text-center <?= ($order['order_status'] === 'delivered' || $order['order_status'] === 'canceled') ? 'd-none' : '' ?>" style="background: linear-gradient(135deg, #101820 0%, #1e293b 100%); border-left: 4px solid #EE2737; border-radius: 16px;">
            <div class="text-white-50 mb-1" style="font-size: 10px; font-weight: 600; letter-spacing: 0.5px;">KODE OTP PENERIMAAN</div>
            <div class="fw-bold my-1" style="letter-spacing: 5px; color: #EE2737; font-size: 24px;"><?= htmlspecialchars($order['otp']) ?></div>
            <div class="text-white-50 mt-1" style="font-size: 10px;">Berikan kode 4-digit ini kepada kurir saat pesanan tiba.</div>
        </div>

        <!-- Driver Info Card (Dynamic Auto-Sync) -->
        <div id="driver-card-container">
            <!-- Assigned Driver Card -->
            <div id="driver-assigned-card" class="p-3.5 bg-white border shadow-xs mb-4 <?= empty($order['delivery_man_id']) ? 'd-none' : '' ?>" style="border-radius: 16px;">
                <div class="d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center gap-3">
                        <img id="driver-avatar-img" src="<?= $baseUrl ?>/<?= htmlspecialchars($order['dm_avatar'] ?? 'assets/images/users/driver.png') ?>" alt="Driver" class="rounded-circle border border-2 border-danger" style="width: 44px; height: 44px; object-fit: cover;">
                        <div>
                            <div class="d-flex align-items-center gap-2">
                                <span id="driver-name-text" class="fw-bold text-dark" style="font-size: 12.5px;"><?= htmlspecialchars($order['dm_name'] ?? 'Mitra Kurir Cicalengka') ?></span>
                                <span class="badge bg-danger-subtle text-danger" style="font-size: 9px;"><i class="bi bi-patch-check-fill me-0.5"></i>Driver</span>
                            </div>
                            <div id="driver-vehicle-text" class="text-muted mt-0.5" style="font-size: 10.5px;">
                    ...
                    <div class="d-flex gap-2 align-items-center">
                        <button type="button" onclick="openChatModal()" class="btn btn-danger btn-sm rounded-pill px-3 py-1.5 d-flex align-items-center justify-content-center shadow-xs position-relative gap-1.5 fw-bold" style="background:#EE2737; border:none; font-size: 11px;" title="Chat Driver">
                            <i class="bi bi-chat-dots-fill text-white" style="font-size: 13px;"></i>
                            <span>Chat</span>
                            <span id="chatUnreadDot" class="ccg-unread-dot d-none"></span>
                        </button>
                        <a id="driver-call-btn" href="tel:<?= htmlspecialchars($order['dm_phone'] ?? '') ?>" class="btn btn-light btn-sm rounded-circle border d-flex align-items-center justify-content-center shadow-xs <?= empty($order['dm_phone']) ? 'd-none' : '' ?>" style="width: 34px; height: 34px;" title="Telepon Kurir">
                            <i class="bi bi-telephone-fill text-dark" style="font-size: 12px;"></i>
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div id="driver-searching-card" class="p-3.5 bg-white border shadow-xs mb-4 <?= !empty($order['delivery_man_id']) ? 'd-none' : '' ?>" style="border-radius: 16px;">
                <div class="d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center gap-3">
                        <div class="spinner-border spinner-border-sm text-danger" style="width: 18px; height: 18px;" role="status"></div>
                        <div>
                            <div class="fw-bold text-dark" style="font-size: 12.5px;">Mencari Kurir Terdekat...</div>
                            <div class="text-muted mt-0.5" style="font-size: 10px;">Sistem sedang menugaskan kurir untuk pesanan Anda</div>
                        </div>
                    </div>
                    <button type="button" onclick="openChatModal()" class="btn btn-outline-danger btn-sm rounded-pill px-3 py-1.5 d-flex align-items-center gap-1.5 fw-bold" style="font-size: 11px;">
        ...
        <div class="p-3.5 bg-white border shadow-xs mb-4" style="border-radius: 16px;">
            <h6 class="fw-bold mb-3 text-dark" style="font-size: 13px;">Status Pengantaran</h6>
            <div class="stepper-container p-0" id="order-stepper">
                ...
                <div class="step-item step-1 completed">
                    ...
                    <div class="flex-grow-1">
                        <div class="fw-bold text-dark" style="font-size: 11.5px;">Pesanan Dikonfirmasi</div>
                        <div class="text-muted" style="font-size: 10px;">Resto/Mitra menerima pesanan Anda</div>
                    </div>
                </div>
                <div class="step-item step-2 <?= $isStep2Done ? 'completed' : ($isStep2Active ? 'active' : '') ?>">
                    ...
                    <div class="flex-grow-1">
                        <div class="fw-bold text-dark" style="font-size: 11.5px;">Diproses & Disiapkan</div>
                        <div class="text-muted" style="font-size: 10px;">Makanan sedang dimasak / dikemas</div>
                    </div>
                </div>
                <div class="step-item step-3 <?= $isStep3Done ? 'completed' : ($isStep3Active ? 'active' : '') ?>">
                    ...
                    <div class="flex-grow-1">
                        <div class="fw-bold text-dark" style="font-size: 11.5px;">Kurir Menuju Lokasi Anda</div>
                        <div class="text-muted" style="font-size: 10px;">Kurir dalam perjalanan pengantaran</div>
                    </div>
                </div>
                <div class="step-item step-4 <?= $isStep4Done ? 'completed' : ($isStep4Active ? 'active' : '') ?>">
                    ...
                    <div class="flex-grow-1">
                        <div class="fw-bold text-dark" style="font-size: 11.5px;">Pesanan Selesai</div>
                        <div class="text-muted" style="font-size: 10px;">Barang sampai dengan aman</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <?php if ($order['order_status'] === 'delivered'): ?>
        ...
        <div class="p-3.5 bg-white border shadow-xs mb-4" id="order-review-section" style="border-radius: 16px;">
            <div class="d-flex align-items-center justify-content-between mb-2.5">
                <div class="d-flex align-items-center gap-2.5">
                    <div class="rounded-circle bg-warning-subtle text-warning d-flex align-items-center justify-content-center" style="width: 34px; height: 34px; font-size: 16px;">
                        <i class="bi bi-star-fill"></i>
                    </div>
                    <div>
                        <h6 class="fw-bold text-dark m-0" style="font-size: 13px;">Ulasan & Rating</h6>
                        <div class="text-muted" style="font-size: 10px;">Penilaian Anda sangat berharga bagi mitra kami</div>
                    </div>
                </div>
                <?php if ($hasReviewed): ?>
                    <span class="badge bg-success-subtle text-success border border-success-subtle px-2.5 py-1 rounded-pill" style="font-size: 9.5px; font-weight: 700;">
            ...
            <?php if ($hasReviewed): ?>
                ...
                <div class="p-3 bg-light rounded-3 border mt-2.5">
                    <?php if (!empty($order['review_info']['store_review'])): ?>
                        <?php $sr = $order['review_info']['store_review']; ?>
                        <div class="mb-2.5">
                            <div class="d-flex justify-content-between align-items-center">
                                <span class="fw-bold text-dark" style="font-size: 11.5px;"><i class="bi bi-shop text-danger me-1"></i> <?= htmlspecialchars($order['store_name'] ?? 'Toko') ?></span>
                            ...
                            <?php if (!empty($sr['comment'])): ?>
                                <div class="text-muted mt-1 ps-1 fst-italic" style="font-size: 10px;">"<?= htmlspecialchars($sr['comment']) ?>"</div>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                    ...
                    <?php if (!empty($order['review_info']['dm_review'])): ?>
                        ...
                        <div class="pt-2.5 border-top">
                            <div class="d-flex justify-content-between align-items-center">
                                <span class="fw-bold text-dark" style="font-size: 11.5px;"><i class="bi bi-bicycle text-primary me-1"></i> <?= htmlspecialchars($order['dm_name'] ?? 'Kurir Driver') ?></span>
                                <div class="text-warning fw-bold" style="font-size: 11px;">
                                    <?php for ($s = 1; $s <= 5; $s++): ?>
                            ...
                            <?php if (!empty($dr['comment'])): ?>
                                <div class="text-muted mt-1 ps-1 fst-italic" style="font-size: 10px;">"<?= htmlspecialchars($dr['comment']) ?>"</div>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                </div>
            <?php else: ?>
                ...
                <div class="p-3 bg-light rounded-3 border mt-2.5">
                    <div class="mb-3">
                        <label class="fw-bold text-dark d-block mb-1.5" style="font-size: 11px;"><i class="bi bi-shop text-danger me-1"></i> Rating Toko / Makanan</label>
                        <div class="d-flex justify-content-center gap-3 py-1.5" id="tracking-store-stars">
                        ...
                        <div class="text-center fw-bold text-warning-emphasis mb-2" id="tracking-store-text" style="font-size: 10.5px;">Sangat Puas (5 Bintang)</div>
                        <textarea id="tracking-store-comment" name="store_comment" class="form-control form-control-sm rounded-2" rows="2" style="font-size: 11px;" placeholder="Bagaimana rasa makanan atau pelayanan toko?"></textarea>
                    </div>

                    <?php if (!empty($order['delivery_man_id'])): ?>
                        <div class="mb-3 pt-2.5 border-top">
                            <label class="fw-bold text-dark d-block mb-1.5" style="font-size: 11px;"><i class="bi bi-bicycle text-primary me-1"></i> Rating Pengantaran Kurir</label>
                            <div class="d-flex justify-content-center gap-3 py-1.5" id="tracking-dm-stars">
                            ...
                            <div class="text-center fw-bold text-warning-emphasis mb-2" id="tracking-dm-text" style="font-size: 10.5px;">Pengantaran Cepat & Ramah (5 Bintang)</div>
                            <textarea id="tracking-dm-comment" name="dm_comment" class="form-control form-control-sm rounded-2" rows="2" style="font-size: 11px;" placeholder="Tuliskan ulasan untuk kurir (opsional)..."></textarea>
                        </div>
                    <?php endif; ?>

                    <button type="button" id="btnSubmitTrackingReview" onclick="submitTrackingReview(<?= (int)$order['id'] ?>, <?= !empty($order['delivery_man_id']) ? 'true' : 'false' ?>)" class="btn btn-danger w-100 rounded-pill fw-bold py-2.5 shadow-xs" style="background:#EE2737; border:none; font-size: 12.5px;">
    ...
    <div class="p-3.5 bg-white border shadow-xs mb-4" style="border-radius: 16px;">
        <h6 class="fw-bold mb-2 text-dark" style="font-size: 12.5px;"><i class="bi bi-shop text-danger me-1"></i> Titik Penjemputan</h6>
        <p class="text-dark fw-semibold mb-0.5" style="font-size: 11.5px;"><?= htmlspecialchars($order['store_name'] ?? 'Penjemputan Parcel') ?></p>
        <p class="text-muted mb-3" style="font-size: 10.5px;"><?= htmlspecialchars($order['store_address'] ?? 'Cicalengka, Bandung') ?></p>

        <h6 class="fw-bold mb-2 text-dark" style="font-size: 12.5px;"><i class="bi bi-geo-alt-fill text-success me-1"></i> Alamat Tujuan Pengantaran</h6>
        <p class="text-muted mb-3" style="font-size: 11px;"><?= htmlspecialchars($order['delivery_address']['address'] ?? 'Cicalengka') ?></p>

        <h6 class="fw-bold mb-2 text-dark" style="font-size: 12.5px;"><i class="bi bi-bag-check-fill text-primary me-1"></i> Rincian Menu / Paket</h6>
        <div class="d-flex flex-column gap-2 text-muted mb-3" style="font-size: 11px;">
        ...
        <hr class="my-2.5">
        <div class="d-flex justify-content-between fw-bold" style="font-size: 12.5px;">
            <span>Total Tagihan (<?= strtoupper($order['payment_method']) ?>)</span>
            <span class="text-danger" style="font-size: 14px;"><?= format_rupiah($order['total_amount']) ?></span>
        </div>
