...
<div class="p-2" style="padding: 6px 10px !important;">
    <h6 class="fw-bold mb-2" style="color: var(--gojek-charcoal); font-size: 11.5px;"><i class="bi bi-egg-fried me-1 text-danger"></i> Daftar Menu Makanan & Minuman</h6>

    <?php if (empty($products)): ?>
        <div class="text-center py-3 text-muted" style="font-size: 10.5px;">Belum ada menu yang ditampilkan.</div>
    <?php else: ?>
        <div class="d-flex flex-column gap-2 mb-3">
            <?php foreach ($products as $prod): ?>
                <div class="p-2 bg-white rounded-3 border shadow-xs d-flex align-items-center justify-content-between gap-2">
                    <div class="flex-grow-1 min-w-0">
                        <div class="fw-bold mb-0.5 text-truncate" style="color: var(--gojek-charcoal); font-size: 11.5px;"><?= htmlspecialchars($prod['name']) ?></div>
                        <div class="text-muted" style="display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; font-size: 9.5px; line-height: 1.25; margin-bottom: 3px;">
                    ...
                    <div class="position-relative flex-shrink-0 text-center">
                        <img src="<?= $baseUrl ?>/<?= htmlspecialchars($prod['image'] ?? 'assets/images/products/default.jpg') ?>" alt="<?= htmlspecialchars($prod['name']) ?>" class="rounded-2" style="width: 58px; height: 58px; object-fit: cover;">
                        <div class="mt-1">
                            <?php if ($store['is_open']): ?>
                                <button type="button" onclick="addToCart(<?= $prod['id'] ?>, 1)" class="btn btn-sm text-white fw-bold px-2 py-0.5 rounded-pill shadow-xs" style="background:#EE2737; font-size: 9.5px;">
                                    + Tambah
                                </button>
                            <?php else: ?>
                                <button type="button" class="btn btn-sm btn-secondary disabled px-2 py-0.5 rounded-pill" style="font-size: 9.5px;">
                                    Tutup
                                </button>
                            <?php endif; ?>
                        </div>
    ...
    <div class="bg-white rounded-3 border shadow-xs p-2 mb-3">
        <div class="d-flex justify-content-between align-items-center mb-2">
            <div class="d-flex align-items-center gap-1.5">
                <div class="rounded-circle bg-warning-subtle text-warning d-flex align-items-center justify-content-center" style="width: 24px; height: 24px; font-size: 11px;">
                    <i class="bi bi-star-fill"></i>
                </div>
                <div>
                    <h6 class="fw-bold m-0 text-dark" style="font-size: 11.5px;">Ulasan & Rating Pembeli</h6>
                    <span class="text-muted" style="font-size: 9.5px;"><?= $store['reviews_count'] ?? 0 ?> ulasan</span>
                </div>
            </div>
            <div class="text-end">
