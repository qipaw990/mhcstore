...
class _DriverProfileScreenState extends State<DriverProfileScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final user = authCtrl.user ?? driverCtrl.driverProfile;

    final name = user?['name'] ?? 'Driver';
    final email = user?['email'] ?? '';
    final phone = user?['phone'] ?? '';
    final rating = driverCtrl.driverRating;
    final totalOrders = driverCtrl.totalOrders;
    final balance = driverCtrl.walletBalance;
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('Kurir Terverifikasi CicalengkaGO', style: TextStyle(color: Colors.white.withOpacity(0.8), fontSize: 11)),
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            _statChip('$totalOrders', 'Total Order'),
                            _statChip(CurrencyFormatter.formatRupiah(balance), 'Saldo'),
                          ],
                        ),
                      ],
                    ),
  ...
  Widget _buildEditForm(DriverController ctrl) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Edit Profil Driver', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Color(0xFF0F172A))),
          const SizedBox(height: 16),
  ...
  Widget _buildMenuSection(BuildContext context, AuthController authCtrl) {
    return Container(
      ...
      child: Column(
        children: [
          _menuItem(Icons.help_outline_rounded, 'Bantuan & FAQ', const Color(0xFF059669), () {}),
          const Divider(height: 1, color: Color(0xFFF1F5F9), indent: 56),
          _menuItem(Icons.info_outline_rounded, 'Ketentuan Mitra Kurir', const Color(0xFF2563EB), () {}),
          const Divider(height: 1, color: Color(0xFFF1F5F9), indent: 56),
          ListTile(
  ...
  void _confirmLogout(BuildContext context, AuthController authCtrl) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('Yakin ingin keluar dari akun Driver CicalengkaGO?'),
        actions: [
